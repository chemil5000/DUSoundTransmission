    ...
    try:
        ...
        if False == is_connected:
            ...
            logging.debug('connecting to {} port {}'.format(
                                  server_address, portRx))
            sockTx.connect(server_address)
            is_connected = True
            data = recv.recvall(sockRx)
            print data
	
    except:
        logging.debug('Unexpected error in send {} '.format(
                                         sys.exc_info()[0]))
...
try:
    ...
    #sock.close()
    print('Unexpected error in main {} '.format(sys.exc_info()[0]))
